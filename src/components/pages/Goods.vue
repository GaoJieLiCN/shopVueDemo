<template>
  <div>
    ...
    <div class="goods-price">价格：{{goodsInfo.price | moneyFilter}}</div>
    <div>
      <van-tabs swipeable sticky>
        <van-tab title="商品详情">
          <!-- <div class="detail" goodsInfo.image></div> -->
          <img :src="goodsInfo.image" width="100%" />
        </van-tab>
...
import goodsInfo from "../json/goodsInfo"; //数据文件
...
export default {
  ...
  methods: {
    getInfo() {
      this.goodsInfo = goodsInfo.data.goodsInfo;
    ...
    addGoodsToCart() {
      ...
      // localStorage.removeItem(cartInfo) //删除缓存数据
